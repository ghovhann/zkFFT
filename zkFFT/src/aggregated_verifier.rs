...
pub fn verify<C: CurveAffine, E: EncodedChallenge<C>, T: TranscriptRead<C, E>>(
    ...
    for _ in 0..n {
        p.push(transcript.read_point().unwrap());
    }
    let s = transcript_e_v(transcript);
    let p = scale_and_sum_points(p, s);
    let mut P_terms: Vec<(<<C as CurveAffine>::CurveExt as CurveExt>::ScalarExt, C)> = vec![(C::Scalar::ONE, p)];
